...
void create_packed_glyph_texture ( Packed_Glyph_Texture &pgt, const char* filename, FT_Library freeType, unsigned int filter )
{
	...
	for ( unsigned char c = startCharacter; c <= endCharacter; ++c )
	{
		...
		tempGlyphs[c].bitmap = new unsigned char [ dimensions ];
	...
	{ 
		combined_character_area += tempGlyphs[i].size.x * tempGlyphs[i].size.y; 
	}

	unsigned int recmDim = sqrt( combined_character_area ) * 1.5;
	recmDim = recmDim + (4 - (recmDim%4));
	pgt.width = recmDim;
	pgt.height = recmDim;
	unsigned char* combinedBitmap = new unsigned char [ recmDim * recmDim ]();
	...
	GLCALL( glTexImage2D( GL_TEXTURE_2D, 0, GL_RED, recmDim, recmDim, 0, GL_RED, GL_UNSIGNED_BYTE, combinedBitmap ) );
	
	GLCALL( glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE ) );
	GLCALL( glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE ) );
	GLCALL( glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, filter ) ); // GL_NEAREST
	GLCALL( glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, filter ) ); // GL_NEAREST
...
void create_text_mesh( const char* text, Text_Mesh &tm, Packed_Glyph_Texture &pgt, unsigned int shader )
{
	...
	{	
		...
		GLCALL( glBindVertexArray( tm.vao ) );
		
			GLCALL( glBindBuffer( GL_ARRAY_BUFFER, tm.vbo_vertices ) );
				GLCALL( glBufferData( GL_ARRAY_BUFFER, verts.size() * sizeof( float ), verts.data(), GL_DYNAMIC_DRAW ) );
			
				GLint posAttrib = GLCALL( glGetAttribLocation( shader, "position" ) );
				GLCALL( glVertexAttribPointer( posAttrib, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0 ) );
				GLCALL( glEnableVertexAttribArray( posAttrib ) );
				
				GLint texAttrib = GLCALL( glGetAttribLocation( shader, "texcoord" ) );
				GLCALL( glVertexAttribPointer( texAttrib, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)(3 * sizeof(float)) ) );
				GLCALL( glEnableVertexAttribArray( texAttrib ) );
			
			GLCALL( glBindBuffer( GL_ARRAY_BUFFER, tm.vbo_colors ) );
				GLCALL( glBufferData( GL_ARRAY_BUFFER, colors.size() * sizeof( unsigned char ), colors.data(), GL_DYNAMIC_DRAW ) );
				
				GLint colorAttrib = GLCALL( glGetAttribLocation( shader, "color" ) );
				GLCALL( glVertexAttribPointer( colorAttrib, 4, GL_UNSIGNED_BYTE, GL_TRUE, 4 * sizeof(unsigned char), (void*)0 ) );
				GLCALL( glEnableVertexAttribArray( colorAttrib ) );

			GLCALL( glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, tm.ebo ) );
				GLCALL( glBufferData( GL_ELEMENT_ARRAY_BUFFER, indis.size() * sizeof( unsigned int ), indis.data(), GL_DYNAMIC_DRAW ) );
...
{
	set_uniform_mat4( shader, "model", &tm.transform ); GLCALL();
	GLCALL( glBindTexture( GL_TEXTURE_2D, tm.texture_id ) );
	GLCALL( glBindVertexArray( tm.vao ) );
	GLCALL( glDrawElements( GL_TRIANGLES, tm.num_indices, GL_UNSIGNED_INT, 0 ) );
}